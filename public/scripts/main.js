...
import { alertCuston, blockSubmit, resetForm, empty } from './auxiliary-functions.js'

$('input:not([type="submit"]):not([type="reset"])').on('blur', function (e) {
	(nome.val() !== empty && email.val() !== empty && phone.val() !== empty)
		?submit.removeAttr('disabled').addClass('cursor-pointer').removeClass('cursor-not-allowed')
		: blockSubmit()
